...
DEFAULT_MODEL = "<evolver-model-id>"
...
class MetaHarnessEngine(EvolutionEngine):
    ...
    def _run_claude_code(self, prompt: str, workspace_root: Path) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                "External coding-agent CLI ('claude') not found on PATH. "
                "This baseline (Meta-Harness) requires a vendor coding-agent "
                "CLI; install it per the vendor's documentation."
            )
            return {
                "output": "",
                "stderr": "external coding-agent CLI not found",
                "exit_code": -1,
            }
