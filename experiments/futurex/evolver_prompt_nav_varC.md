...
- Main branch = shared search infrastructure (tools, registry). Keep it stable.
- Domain-specific strategies (prompts, skills, memory) belong on BRANCHES.
- When evolving main: only touch tools/ and tools/registry.yaml.
- When evolving a branch: focus on prompts/system.md, skills/, and memory/.
- Do not rewrite prompts/system.md from scratch — make targeted additions.
- If tools work, preserve them.
