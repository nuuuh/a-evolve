...
The analytical decomposition identifies $\Ladapt$ as the loss from committing to one dense harness before observing the incoming task's context. We reduce this loss by shifting heavy adaptation into evolution time: the evolver constructs a \emph{harness tree} of specialized harnesses, and solve time only requires a lightweight routing decision (Figure~\ref{fig:unified_pipeline}, top panel). This design targets task heterogeneity and non-stationarity, where a single dense harness can underfit specialized task contexts even when the evolver is capable.
