...
An agent's harness, comprising the prompts, skills, tools, and supporting infrastructure that surround a fixed LLM, is a primary determinant of task-solving performance. \emph{Auto-harness systems} such as A-Evolve~\cite{lin2026position}, GEPA~\cite{agrawal2025gepa}, and Meta-Harness~\cite{lee2026meta} performs evolution to construct harness automatically from execution feedback, and report substantial gains on static offline benchmarks such as SWE-bench~\cite{jimenez2024swe}. However, online real-world deployment, whose tasks and evaluations follow a strict chronologic order, demands \emph{sustained self-improvement}: a sustained improvement of the harness as the agent operates on an open-ended task stream rather than on a fixed evaluation set. 
...
\textbf{Bottom:} pass-rate lift for evolution with fixed cycles on Polymarket trading tasks overtime.}
\label{fig:c1}
% \vskip -1.5em
\end{figure}
...
A concrete deployment failure illustrates the gap. As shown in Figure~\ref{fig:c1}, we perform A-evolve on polymarket trading tasks across weeks with different evolution budgets (evolution stops after N cycles). While evolution initially improves the harness (lift compared to no evolution), the runs with more budget accumulates source preferences and tools growth that consume context and bias later tasks. Across all five evolution-budget settings, every curve peaks early and declines, with fewer budget runs beating those with more evolutions, which indicates achieving sustained improvement on open-ended task stream requires more than continual update with more cycles.

Therefore, as illustrated in Figure~\ref{fig:teaser}, we point out three deployment dimensions that distinguish the two settings. \textbf{(D1) Unbounded Streams.} The task stream has no fixed train/test cutoff or designated endpoint; feedback, trajectories, and harness state keep accumulating during deployment, creating heavy burden for the evolution.
...
Existing auto-harness systems built around a single-agent evolver are bounded by a finite context window, leaving little room to approach the large and growing volume of trajectories that effective and generalizable harness would require to be built. \textbf{(D2) Task heterogeneity.} The stream contains tasks of varied types that demand different harness components. A prediction-market platform, for example, mixes politics, sports, and finance questions in the same hour, each calling for distinct sources, tools, and prompting patterns. This rationale parallels a more general phenomenon: a single fixed policy is rarely optimal across heterogeneous problems~\cite{miao2025reinforcement}.
{Existing auto-harness systems analogously deploy a static dense harness across the stream, with no solve-time adaptation to the task at hand.} \textbf{(D3) Distributional non-stationarity.} As the stream progresses, incoming tasks shift away from the experience the harness was last fitted on. A harness optimised for recent cycles therefore drifts out of fit for new tasks, even with rich historical experience. Closing this gap requires per-task contextual adaptation of the harness, not only continued historical fitting. Additional diagnostics are shown in Appendix~\ref{app:benchmarks}.
